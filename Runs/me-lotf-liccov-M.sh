...
mkdir -p Output/$OUTPUTDIR/M$INSTANCES
cd Output/$OUTPUTDIR/M$INSTANCES

while [ $count -le $RUNS ]
do
    echo Run for maxPortion = $count:
    evaluate /raid/farago/Experiments/Skeletons/Gen3_Bare_LazyOTF.jtorx  /raid/farago/Experiments/Skeletons/licenseGenerator3Skel.sax $count $MAXSTEP $SAMPLE "liccov-lazyotfto;0;$count;use-dist-mudp;mudp-loopback" lazyotf
    count=`expr $count + 1`
done
